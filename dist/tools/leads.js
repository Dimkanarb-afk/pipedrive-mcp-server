...
function formatLead(lead) {
    ...
    if (lead.person_id)
        lines.push(`- **Person ID**: ${lead.person_id}`);
    if (lead.organization_id)
        lines.push(`- **Organization ID**: ${lead.organization_id}`);
    if (lead.source_name)
        lines.push(`- **Source**: ${lead.source_name}`);
    if (lead.label_ids?.length)
        lines.push(`- **Labels**: ${lead.label_ids.join(', ')}`);
    if (lead.expected_close_date)
        lines.push(`- **Expected Close**: ${lead.expected_close_date}`);
    lines.push(`- **Archived**: ${lead.is_archived ? 'Yes' : 'No'}`, `- **Added**: ${lead.add_time} | **Updated**: ${lead.update_time}`);
    return lines.join('\n');
}
// ─── Tool Registration ────────────────────────────────────────────────────────
function registerLeadTools(server, client) {
    // ── pipedrive_list_leads ──────────────────────────────────────────────────
    (0, utils_1.registerTool)(server, 'pipedrive_list_leads', {
...
Don't use when: Searching for a lead by keyword (use pipedrive_search_leads).`,
        inputSchema: {
            ...
            archived_status: zod_1.z
                .enum(['archived', 'not_archived', 'all'])
                .optional()
                .describe('Filter by archive status. Default: not_archived'),
            owner_id: zod_1.z.number().int().positive().optional().describe('Filter by owner user ID'),
            person_id: zod_1.z.number().int().positive().optional().describe('Filter by linked person/contact ID'),
            organization_id: zod_1.z.number().int().positive().optional().describe('Filter by linked organization ID'),
        },
        ...
        try {
            ...
            if (params.archived_status)
                queryParams.archived_status = params.archived_status;
            if (params.owner_id)
            ...
            const response = await client.get('/leads', queryParams);
            ...
            if (leads.length === 0)
                text += '_No leads found matching the given filters._';
            else
                text += leads.map(formatLead).join('\n\n');
            ...
            return (0, pipedrive_1.handlePipedriveError)(error, 'list leads');
        }
    });
    // ── pipedrive_get_lead ────────────────────────────────────────────────────
    (0, utils_1.registerTool)(server, 'pipedrive_get_lead', {
...
Don't use when: You don't know the ID (use pipedrive_search_leads or pipedrive_list_leads).`,
        inputSchema: {
            id: zod_1.z.string().uuid().describe('Lead UUID to retrieve (e.g. "adf21080-0e10-11eb-879b-05d71fb426ec")'),
        },
        annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: true },
    }, async (raw) => {
        const { id } = raw;
        try {
            ...
            return {
                content: [{ type: 'text', text: `# Lead: ${lead.title}\n\n${formatLead(lead)}` }],
                structuredContent: lead,
            };
        }
        catch (error) {
            return (0, pipedrive_1.handlePipedriveError)(error, `get lead ${id}`);
        }
    });
    // ── pipedrive_create_lead ─────────────────────────────────────────────────
    (0, utils_1.registerTool)(server, 'pipedrive_create_lead', {
...
Returns: The newly created lead with assigned UUID.`,
        inputSchema: {
            title: zod_1.z.string().min(1).describe('Lead title/name (required)'),
            owner_id: zod_1.z.number().int().positive().optional().describe('Owner user ID. Defaults to authenticated user'),
            label_ids: zod_1.z.array(zod_1.z.string()).optional().describe('Array of lead label UUIDs. Get labels with pipedrive_get_lead_labels'),
            person_id: zod_1.z.number().int().positive().optional().describe('Link to an existing person/contact ID'),
            organization_id: zod_1.z.number().int().positive().optional().describe('Link to an existing organization ID'),
            value: zod_1.z.number().min(0).optional().describe('Monetary value estimate'),
            currency: zod_1.z.string().length(3).optional().describe('Currency code (3 letters, e.g. USD, EUR). Default: USD'),
        ...
        try {
            ...
            const lead = response.data;
            return {
                content: [{ type: 'text', text: `# Lead Created Successfully\n\n${formatLead(lead)}` }],
                structuredContent: lead,
            };
        }
        catch (error) {
            return (0, pipedrive_1.handlePipedriveError)(error, 'create lead');
        }
    });
    // ── pipedrive_update_lead ─────────────────────────────────────────────────
    (0, utils_1.registerTool)(server, 'pipedrive_update_lead', {
...
Returns: Updated lead object.`,
        inputSchema: {
            id: zod_1.z.string().describe('Lead UUID to update (required)'),
            title: zod_1.z.string().min(1).optional().describe('New lead title'),
            owner_id: zod_1.z.number().int().positive().optional().describe('New owner user ID'),
            label_ids: zod_1.z.array(zod_1.z.string()).optional().describe('New label UUID array (replaces existing labels)'),
            person_id: zod_1.z.number().int().positive().optional().describe('New linked person/contact ID'),
            organization_id: zod_1.z.number().int().positive().optional().describe('New linked organization ID'),
            value: zod_1.z.number().min(0).optional().describe('New monetary value estimate'),
            currency: zod_1.z.string().length(3).optional().describe('New currency code (3 letters)'),
            expected_close_date: zod_1.z.string().optional().describe('New expected close date (YYYY-MM-DD)'),
            is_archived: zod_1.z.boolean().optional().describe('Archive (true) or unarchive (false) this lead'),
        },
        annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: false, openWorldHint: true },
    }, async (raw) => {
        const params = raw;
        try {
            ...
            if (rest.owner_id !== undefined)
                body.owner_id = rest.owner_id;
            if (rest.label_ids !== undefined)
                body.label_ids = rest.label_ids;
            if (rest.person_id !== undefined)
                body.person_id = rest.person_id;
            if (rest.organization_id !== undefined)
                body.organization_id = rest.organization_id;
            if (rest.expected_close_date !== undefined)
                body.expected_close_date = rest.expected_close_date;
            if (rest.is_archived !== undefined)
                body.is_archived = rest.is_archived;
            if (value !== undefined) {
                body.value = { amount: value, currency: currency ?? 'USD' };
            }
            const response = await client.patch(`/leads/${id}`, body);
            const lead = response.data;
            return {
                content: [{ type: 'text', text: `# Lead Updated Successfully\n\n${formatLead(lead)}` }],
                structuredContent: lead,
            };
        }
        catch (error) {
            return (0, pipedrive_1.handlePipedriveError)(error, `update lead ${params.id}`);
        }
    });
    // ── pipedrive_delete_lead ─────────────────────────────────────────────────
    (0, utils_1.registerTool)(server, 'pipedrive_delete_lead', {
...
Warning: This permanently deletes the lead. Consider archiving (via pipedrive_update_lead
  with is_archived: true) if you may need the data later.`,
        inputSchema: {
            id: zod_1.z.string().describe('Lead UUID to permanently delete (required)'),
        },
        ...
        try {
            ...
            return {
                content: [{ type: 'text', text: `# Lead Deleted\n\nLead \`${id}\` has been permanently deleted.` }],
                structuredContent: { deleted: true, id },
            };
        }
        catch (error) {
            return (0, pipedrive_1.handlePipedriveError)(error, `delete lead ${id}`);
        }
    });
    // ── pipedrive_search_leads ────────────────────────────────────────────────
    (0, utils_1.registerTool)(server, 'pipedrive_search_leads', {
...
Use when: Looking for a lead by name or associated entity without knowing the exact ID.`,
        ...
        const start = params.start ?? 0;
        try {
            const response = await client.get('/leads/search', { term: params.term, limit, start });
            ...
            if (items.length === 0) {
                ...
                for (const { result_score, item } of items) {
                    ...
                    text += `- **ID**: ${item.id}\n`;
                    if (item.person)
                        text += `- **Person**: ${item.person.name} (ID: ${item.person.id})\n`;
            ...
            return (0, pipedrive_1.handlePipedriveError)(error, `search leads for "${params.term}"`);
        }
    });
    // ── pipedrive_get_lead_labels ─────────────────────────────────────────────
    (0, utils_1.registerTool)(server, 'pipedrive_get_lead_labels', {
...
Also useful for understanding how leads are categorized in the account.`,
        ...
        try {
            ...
            return (0, pipedrive_1.handlePipedriveError)(error, 'get lead labels');
        }
    });
    // ── pipedrive_get_lead_sources ────────────────────────────────────────────
    (0, utils_1.registerTool)(server, 'pipedrive_get_lead_sources', {
...
or to filter/report on leads by their origin channel.`,
        ...
        try {
            ...
            if (sources.length === 0)
                text += '_No lead sources configured._';
            else {
            ...
            return (0, pipedrive_1.handlePipedriveError)(error, 'get lead sources');
        }
    });
}
